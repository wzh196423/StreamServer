var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { Http, RequestOptions, Headers } from '@angular/http';
import { User } from '../entities/user';
var LocalUserService = /** @class */ (function () {
    function LocalUserService(http) {
        this.http = http;
        this.localUser = new User('Me', 'Who am I');
    }
    LocalUserService.prototype.getLocalUser = function () {
        return this.localUser;
    };
    LocalUserService.prototype.setLocalUser = function (user) {
        this.localUser = user;
    };
    LocalUserService.prototype.modifyNickname = function (nickname) {
        var _this = this;
        var headers = new Headers({ 'Content-Type': 'application/json' });
        var options = new RequestOptions({ headers: headers });
        var url = 'http://120.25.238.161:3000/user/modifyNickname';
        var info = {
            username: this.localUser.username,
            nickname: nickname,
        };
        return this.http.put(url, JSON.stringify(info), options)
            .toPromise()
            .then(function (res) {
            if (res.json().data === 'success') {
                _this.localUser.nickname = nickname;
                return Promise.resolve('success');
            }
            return Promise.resolve('error');
        }).catch(function (error) {
            console.log('LocalUserService-modifyNickname', error);
            return Promise.resolve('error');
        });
    };
    LocalUserService = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [Http])
    ], LocalUserService);
    return LocalUserService;
}());
export { LocalUserService };
//# sourceMappingURL=local-user.service.js.map